  ...
  const rclcpp::Time & target_ros_time)
{
...

  {
    ...
    //   return std::nullopt;
    // }
    // RCLCPP_INFO(logger_, "this time is : %ld", target_ros_time.nanoseconds());
    // RCLCPP_INFO(logger_, "this time_first is : %ld", time_first.nanoseconds());

    // [time_last < target_ros_time] is acceptable here.s
