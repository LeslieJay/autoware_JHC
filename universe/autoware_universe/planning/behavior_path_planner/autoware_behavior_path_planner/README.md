...
Essentially, the module has three primary responsibilities 该模块主要有以下三项职责: 

1. Creating a **path based** on the traffic situation.根据交通状况创建一条行驶路径。
2. Generating **drivable area**, i.e. the area within which the vehicle can maneuver.生成可行驶区域，即车辆可以进行机动操作的区域。
3. Generating **turn signal** commands to be relayed to the vehicle interface.生成转向信号指令，并将其传递给车辆接口。
