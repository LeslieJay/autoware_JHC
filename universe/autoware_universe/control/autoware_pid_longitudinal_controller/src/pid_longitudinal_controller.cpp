...
/**
 * @brief 主要纵向控制命令计算函数，根据当前状态和目标轨迹计算最终的速度/加速度控制命令
 * 
 * @param control_data 控制数据，包括插值后轨迹点、当前状态、延迟补偿等
 * @return Motion 结构体，包含 velocity/acceleration 指令
 */
  ...
  const ControlData & control_data)
{
  ...
  // 初始化控制指令（油门/制动器的期望值），初值来自插值轨迹点的速度和加速度
  ...
  RCLCPP_INFO_THROTTLE(logger_, *clock_, 5000, "ctrl_cmd_as_pedal_pos: vel=%.2f, acc=%.2f", 
  ...
  if (m_control_state == ControlState::STOPPED) {
    ...
    RCLCPP_INFO_THROTTLE(logger_, *clock_, 5000, "m_control_state == ControlState::STOPPED");
    // 直接使用设定的停止速度/加速度参数
    ...
    // ================================
    // DRIVE / STOPPING / EMERGENCY 状态综合处理
    // ================================
    Motion raw_ctrl_cmd{
      control_data.interpolated_traj.points.at(target_idx).longitudinal_velocity_mps,
      control_data.interpolated_traj.points.at(target_idx).acceleration_mps2
    ...
    if (m_control_state == ControlState::EMERGENCY) {
      ...
      // --- 纵向加速度限幅 ---
      raw_ctrl_cmd.acc =
        std::clamp(raw_ctrl_cmd.acc, m_min_acc, m_max_acc);
      m_debug_values.setValues(DebugValues::TYPE::ACC_CMD_ACC_LIMITED, raw_ctrl_cmd.acc);
  ...
  // ================================
  // 加速度最后一轮指令输出斜率限幅（突变保护），与上次发布指令限幅
  // ================================
