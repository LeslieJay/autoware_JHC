...
void PointCloudConcatenationComponent::checkSyncStatus()
{
  ...
  RCLCPP_INFO(get_logger(), "Starting checkSyncStatus...");

  for (const auto & e : cloud_stdmap_) {
    if (e.second != nullptr) {
      const auto stamp = rclcpp::Time(e.second->header.stamp);
      RCLCPP_DEBUG_STREAM(
        get_logger(), "Topic: " << e.first << ", Stamp: " << stamp.nanoseconds());

      auto it = std::find(pc_stamps.begin(), pc_stamps.end(), stamp);
      if (it != pc_stamps.end()) {
        // found
        RCLCPP_DEBUG_STREAM(get_logger(), "Duplicate stamp found for topic: " << e.first);
        continue;
      } else {
        // not found
        pc_stamps.push_back(stamp);
        RCLCPP_DEBUG_STREAM(
          get_logger(), "Added new stamp for topic: " << e.first << ", Stamp: " << stamp.nanoseconds());
      }
    } else {
      RCLCPP_DEBUG_STREAM(get_logger(), "Topic: " << e.first << " has nullptr cloud.");
    }
  }

  // 打印 pc_stamps 的大小
  RCLCPP_INFO_STREAM(get_logger(), "pc_stamps.size(): " << pc_stamps.size());

  // 1. Check if all stamps are same
  if (pc_stamps.size() == 1) {
    RCLCPP_INFO(get_logger(), "All stamps are identical. No further action needed.");
    return;
  }
  ...
  for (const auto & e : cloud_stdmap_tmp_) {
    if (e.second != nullptr) {
      const auto stamp = rclcpp::Time(e.second->header.stamp);
      RCLCPP_DEBUG_STREAM(
        get_logger(), "Tmp Topic: " << e.first << ", Stamp: " << stamp.nanoseconds());

      auto it = std::find(pc_stamps.begin(), pc_stamps.end(), stamp);
      if (it != pc_stamps.end()) {
        // found
        RCLCPP_DEBUG_STREAM(get_logger(), "Duplicate stamp found in tmp for topic: " << e.first);
        continue;
      } else {
        // not found
        pc_stamps.push_back(stamp);
        RCLCPP_DEBUG_STREAM(
          get_logger(), "Added new stamp from tmp for topic: " << e.first << ", Stamp: " << stamp.nanoseconds());
      }
    } else {
      RCLCPP_DEBUG_STREAM(get_logger(), "Tmp Topic: " << e.first << " has nullptr cloud.");
    }
  }

  // sort pc_stamps
  std::sort(pc_stamps.begin(), pc_stamps.end());
  RCLCPP_DEBUG_STREAM(get_logger(), "Sorted pc_stamps: ");
  for (const auto & stamp : pc_stamps) {
    RCLCPP_DEBUG_STREAM(get_logger(), "Stamp: " << stamp.nanoseconds());
  }

  // restrict the size of pc_stamps to newer 2 stamps
  if (pc_stamps.size() > 2) {
    pc_stamps.erase(pc_stamps.begin(), pc_stamps.end() - 2);
    RCLCPP_DEBUG_STREAM(get_logger(), "Restricted pc_stamps to latest 2 stamps.");
  }
  ...
  for (auto & e : cloud_stdmap_) {
    ...
    if (cloud_stdmap_tmp_[e.first] == nullptr) {
      RCLCPP_DEBUG_STREAM(get_logger(), "Tmp cloud for topic: " << e.first << " is nullptr.");
      continue;
    }

    const auto next_stamp = rclcpp::Time(cloud_stdmap_tmp_[e.first]->header.stamp);
    RCLCPP_DEBUG_STREAM(
      get_logger(), "Checking tmp cloud for topic: " << e.first << ", Stamp: " << next_stamp.nanoseconds());

    if (next_stamp == pc_stamps.front()) {
      RCLCPP_DEBUG_STREAM(
        get_logger(), "Moving tmp cloud back to cloud_stdmap_ for topic: " << e.first);
      e.second = cloud_stdmap_tmp_[e.first];
      cloud_stdmap_tmp_[e.first] = nullptr;
    } else if (next_stamp == pc_stamps.back()) {
      RCLCPP_DEBUG_STREAM(
        get_logger(), "Keeping tmp cloud in cloud_stdmap_tmp_ for topic: " << e.first);
    } else {
      // this state should not be reached. discard data
      RCLCPP_WARN_STREAM(
        get_logger(), "Unexpected tmp stamp. Discarding tmp data for topic: " << e.first);
      cloud_stdmap_tmp_[e.first] = nullptr;
    }
  }

  RCLCPP_INFO(get_logger(), "Finished checkSyncStatus.");
  return;
}
