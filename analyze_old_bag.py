...
def analyze_old_bag(bag_path):
    ...
    plt.plot(df['t'] - df['t'].iloc[0], np.degrees(df['yaw']), 'g-', label='Old RTK Yaw')
    plt.title('Old RTK: Yaw Orientation (Comparison Baseline)')
    plt.xlabel('Time [s]')
    plt.ylabel('Yaw [deg]')
    plt.grid(True, alpha=0.3)
    plt.legend()
    plt.savefig('old_rtk_yaw.png')
    print("Plot saved to old_rtk_yaw.png")
    ...
    print(f"  Non-physical jumps detected (>300 deg/s): {len(jumps)}")

if __name__ == "__main__":
    analyze_old_bag(sys.argv[1])
